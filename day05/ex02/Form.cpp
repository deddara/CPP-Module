...
Form::Form(std::string name, int sign_req, int sign_exec, std::string target) : _name(name), _signed(0), _sign_exec (sign_exec), _sign_req(sign_req), _target(target) {
...
_sign_req(cp._sign_req){ *this = cp; }

Form::~Form() {return ;}

Form & Form::operator=(Form const &sec) {
	_target = sec._target;
	return (*this);
}
